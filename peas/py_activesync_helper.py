...
def search_emails(creds, keywords, max_results=50):
    try:
        all_emails_xml = extract_emails(creds)
    except Exception:
        return []
    ...
    for xml in all_emails_xml:
        
        subject = ''
        subj_match = re.search(r'<[^>]*Subject[^>]*>([^<]*)</', xml)
        if subj_match:
            subject = subj_match.group(1).strip()
        
        from_addr = ''
        from_match = re.search(r'<[^>]*From[^>]*>(.*?)</', xml, re.DOTALL)
        if from_match:
            raw_from = from_match.group(1).replace('"', '').strip()
            
            if ' <' in raw_from:
                from_addr = raw_from.split(' <')[0]
            else:
                from_addr = raw_from
        
        date_str = ''
        date_match = re.search(r'<[^>]*DateReceived[^>]*>([^<]*)</', xml)
        if date_match:
            date_str = date_match.group(1).strip()
       
        body_text = ''
        data_match = re.search(r'<airsyncbase:Data[^>]*>(.*?)</airsyncbase:Data>', xml, re.DOTALL)
        if data_match:
            raw_data = data_match.group(1)
            body_text = re.sub(r'<[^>]+>', '', raw_data)  
